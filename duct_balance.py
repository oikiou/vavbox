...
for i in range(30000):
    ...
    # p2 - g2
    g2 = cal_p0_to_g2(p2, s, se, cv2, k)

    # g2 - p3
    p3 = p2 + 2 * s * (g1 + g2) * (g1 + g2)

    # p3 - g3
    g3 = cal_p0_to_g2(p3, s, se, cv3, k)

    if g3 > 6000:
        continue

    # g3 - p
    p = p3 + 2 * s * (g1 + g2 + g3) * (g1 + g2 + g3)

    if p > 600:
        continue

    # p - inv
    pi1 = g_to_p_fan((g1 + g2 + g3), 1, fan_2_performance)
    inv = p / pi1
